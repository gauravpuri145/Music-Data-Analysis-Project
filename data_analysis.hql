...
PARTITION(batchid=${hiveconf:batchid})
SELECT 
CASE WHEN (su.user_id IS NULL OR CAST(ed.u_timestamp AS DECIMAL(20,0)) > CAST(su.subscn_end_dt AS DECIMAL(20,0))) THEN 'UNSUBSCRIBED'
WHEN (su.user_id IS NOT NULL AND CAST(ed.u_timestamp AS DECIMAL(20,0)) <= CAST(su.subscn_end_dt AS DECIMAL(20,0))) THEN 'SUBSCRIBED'
END AS user_type,
...
GROUP BY CASE WHEN (su.user_id IS NULL OR CAST(ed.u_timestamp AS DECIMAL(20,0)) > CAST(su.subscn_end_dt AS DECIMAL(20,0))) THEN 'UNSUBSCRIBED'
WHEN (su.user_id IS NOT NULL AND CAST(ed.u_timestamp AS DECIMAL(20,0)) <= CAST(su.subscn_end_dt AS DECIMAL(20,0))) THEN 'SUBSCRIBED' END;
...
WHERE status='pass'
AND batchid=${hiveconf:batchid}
AND (u_like=1 OR song_end_type=0)
GROUP BY song_id
